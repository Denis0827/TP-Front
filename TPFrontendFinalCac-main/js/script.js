...
function miFunction() {
    ...
    const resultado = leoDatosDelApi(usuario, contrasena);                                //verifico si existe el mail contra la API
    /* alert('Freno para leer el api porque no se como esperar a que el promise se haga efectivo.'); */
    /* if (typeof(resultado).tostring == 'object'){
        alert ('resulto object');
        alert(typeof(resultado)+' 1');
    }else{
        alert(resultado + ' 2');
    }; */

   /*  while (typeof(resultado) == ) */
...
function validaContrasena(contrasena){  //Vamos a buscar al menos 1 mayuscula, 1 minuscula, 1 numero y 6 caracteres
...
function manejarErrores(elementId, errorId, esValido) {
    ...
    if (!esValido) { //si la variable esvalido es falso
...
async function leoDatosDelApi(usuario, contrasena) {
    ...
    const response = await fetch('https://reqres.in/api/login', {
        ...
        /* Desde aca probamos - ANDA */
        /* .then(
            function (u) { return u.json(); }
        )
        .then(
            await Promise.all (function (json) {
                data_function(json); //llama y pasa json a otra funcion
